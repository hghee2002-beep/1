...
## D-035. 참가자 실명 상시 공개와 PUUID 중심 Riot 검증

- 배경: 소규모 대회 운영 정책상 공개 순위와 참가자 프로필에서 모든 참가자의 실명을 표시하며, 참가 신청의 공개 선택 UI가 필요하지 않다. 또한 Riot ID의 gameName은 내부 공백을 허용하지만 Summoner-V4의 legacy encrypted summoner ID가 누락된 정상 응답이 신청 검증을 차단할 수 있다.
