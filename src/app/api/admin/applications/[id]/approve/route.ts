...
import { approveParticipationApplication } from "@/server/applications/service";
...
export async function POST(request: NextRequest, context: Context) {
  ...
  try {
    ...
    const result = await approveParticipationApplication({
      applicationId: id,
    ...
    console.error("admin.application.approve.failed");
    return authErrorResponse({
      code: "INTERNAL_ERROR",
      message: "참가 신청을 승인하지 못했습니다.",
      status: 500,
    });
  }
}
