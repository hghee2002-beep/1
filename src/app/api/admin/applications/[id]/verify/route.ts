...
import {
  ...
  reverifyApplicationInputSchema,
} from "@/features/applications/validation";
import {
  applicationRouteErrorResponse,
  requestIdFrom,
} from "@/server/applications/http";
import { reverifyParticipationApplication } from "@/server/applications/service";
...
export async function POST(request: NextRequest, context: Context) {
  ...
  try {
    ...
    const parsed = reverifyApplicationInputSchema.safeParse(
    ...
    const result = await reverifyParticipationApplication({
      applicationId: id,
    ...
    console.error("admin.application.verify.failed");
    return authErrorResponse({
      code: "INTERNAL_ERROR",
      message: "참가 신청을 재검증하지 못했습니다.",
      status: 500,
    });
  }
}
